...
- **§146** — [A validity check applied uniformly across categories silently DELETED a whole category, and the surface it emptied looked healthy](lessons/146-a-validity-check-applied-uniformly-deleted-a-whole-catego.md)
- **§147** — [Node strips TypeScript itself now, so a TypeScript project can carry ZERO runtime dependencies and one build one](lessons/147-node-strips-typescript-itself-so-a-project-can-carry-zero.md)
- **§148** — [A gate placed after an expensive optional step is a gate that stops running when that step breaks](lessons/148-a-gate-after-an-expensive-optional-step-stops-running-whe.md)
- **§149** — [A generated file that is gitignored makes CI the first fresh clone the repository has ever had](lessons/149-a-generated-gitignored-file-makes-ci-the-first-fresh-clon.md)
- **§150** — [Every secondary button in a shipped app was painted by the browser, not by the theme — and every gate was green](lessons/150-every-secondary-button-was-painted-by-the-browser-not-the.md)
- **§151** — [An app offered the reader a new version, they took it, and nothing ever said what changed](lessons/151-an-app-offered-an-update-and-never-said-what-it-changed.md)
- **§152** — [A check's sentence and its predicate are two different things, and only one of them runs](lessons/152-a-checks-sentence-and-its-predicate-are-two-different-thi.md)
- **§153** — [A button class that omits `display` means something else on an `<a>`, and the floor it declares is silently inert](lessons/153-a-button-class-that-omits-display-means-something-else-on.md)
