...
- **§322** — [A preview at screen scale hides an export-scale defect, and a statistic on the population you were fixing cannot see what you broke outside it — and a fix that leaves the instrument's reading unchanged has not touched the cause](lessons/322-a-preview-at-screen-scale-hides-an-export-scale-defect.md)
- **§323** — [A harness default that nothing overrides is a policy nobody chose, and a link from a public repo to a private chat is a quotation of the whole chat](lessons/323-a-harness-default-that-nothing-overrides-is-a-policy-nobody-chose.md)
- **§324** — [A toggle pressed blind alternates, so a render is captioned with the control state read back, never the state it asked for](lessons/324-a-toggle-pressed-blind-alternates.md)
...
- **§326** — [A look is built with the app's own tools through its own selections, and the hand-made result is the specification — a new stage is the last resort, not the first](lessons/326-a-look-is-built-with-the-apps-own-tools-through-its-own-selections.md)
