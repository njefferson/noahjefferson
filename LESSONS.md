...
- **§354** — [A new format inherits a rule from the one module whose whole subject is the case the new format does not have — and inherits the prose with it, so the limit gets written down twice as though it had been decided](lessons/354-a-new-format-inherits-a-rule-from-the-one-module-whose-subject-is-the-case-it-does-not-have.md)
