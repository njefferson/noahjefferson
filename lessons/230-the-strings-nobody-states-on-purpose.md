...
every run. · GATE noahjefferson:essay.mjs — an essay declares `author`, the
byline is DERIVED from it, an unknown key is a failure and there is no default. ·
CHECKLIST invent-the-example-never-borrow-it — when writing sample copy, invent
the scenario; the nearest real one is the one in front of you and it is somebody's.
...
list. The author gate was watched refuse an unknown key.
